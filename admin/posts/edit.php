            ...
            <div class="row add-post"><!--подгужаем файлы во временную память нашего сервера-->
                ...
                <form action="edit.php" enctype="multipart/form-data" method="post">
                    ...
                    <select name="section" class="form-select mb-4" aria-label="Default select example">
                        ...
                        <?php if ($value['name'] == $topic['name']):?>
                            <option value="<?= $value['id']?>" selected><?=$topic['name'] ?></option>
                        <?php else:?>
                    ...
                    <div class="col">
                        <button name="edit_post" class="btn btn-primary mb-4" type="submit">Добавить запись</button>
                    </div>
                </form>
            </div>
        </div>
